...
const en: Record<string, string> = {
  ...
  "会话数据已复制，目标会话可以继续使用": "Session data copied. The target session is ready to continue.",
  ...
  "复制后会保留目标会话的新身份，但目标会话现有内容将被源会话历史覆盖；源会话不会改变，并会自动备份目标会话。":
    "The target session keeps its new identity, but its current content will be replaced by the source history. The source stays unchanged, and the target is backed up automatically.",
  "已删除": "Deleted",
...
const ru: Record<string, string> = {
  ...
  "会话数据已复制，目标会话可以继续使用": "Данные скопированы, целевую сессию можно продолжить.",
  ...
  "复制后会保留目标会话的新身份，但目标会话现有内容将被源会话历史覆盖；源会话不会改变，并会自动备份目标会话。":
    "Целевая сессия сохранит новый идентификатор, но ее содержимое будет заменено историей исходной сессии. Исходная сессия не изменится, а целевая будет автоматически сохранена в резервную копию.",
  "已删除": "Удалено",
...
const simplifiedToTraditional: Record<string, string> = {
  ...
  "会话数据已复制，目标会话可以继续使用": "會話資料已複製，目標會話可以繼續使用",
  ...
  "修改会话名称失败": "修改會話名稱失敗",
  "复制后会保留目标会话的新身份，但目标会话现有内容将被源会话历史覆盖；源会话不会改变，并会自动备份目标会话。":
    "複製後會保留目標會話的新身分，但目標會話現有內容將被來源會話歷史覆蓋；來源會話不會改變，並會自動備份目標會話。",
